...
export default function PublicLayout({ children }: PublicLayoutProps) {
  ...
  const isHomePage = location === '/' || location === '/public';
  ...
  // Buscar último evento cadastrado em tempo real - apenas na página inicial
  const { data: latestEvent } = useQuery({
    queryKey: ['/api/public/events/latest'],
    refetchInterval: isHomePage ? 30 * 1000 : false, // Atualizar apenas na página inicial
    staleTime: 15 * 1000, // Considerar stale após 15 segundos
    enabled: isHomePage, // Executar query apenas na página inicial
  });
    ...
    <div className={`min-h-screen flex flex-col ${isDarkMode ? 'dark bg-slate-900 text-white' : 'bg-white'} ${isHighContrast ? 'high-contrast' : ''}`}>
      ...
      <div className="hidden md:block fixed top-40 left-1/2 transform -translate-x-1/2 z-40 w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex justify-end">
          <div className={`flex items-center space-x-2 sm:space-x-3 transition-all duration-300 ease-in-out ${
            ...
            {/* Componente Último Evento - apenas na página inicial */}
            {isHomePage && (
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border border-green-200/50 dark:border-green-700/50 rounded-lg px-2 py-1.5 sm:px-3 sm:py-2 shadow-lg hover:shadow-xl transition-all duration-300 max-w-[180px] sm:max-w-[220px] lg:max-w-[250px]">
                <div className="flex items-center space-x-1.5 sm:space-x-2">
                  <div className="flex-shrink-0">
                    <div className="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 bg-green-100 dark:bg-green-800 rounded-full flex items-center justify-center">
                      <Calendar size={12} className="sm:w-3.5 sm:h-3.5 lg:w-4 lg:h-4 text-green-600 dark:text-green-400" />
                    </div>
                  </div>
                    ...
                    {latestEvent && typeof latestEvent === 'object' && 'title' in latestEvent ? (
                      <Link href="/sessoes">
                        <div className="cursor-pointer group">
                          <p className="text-xs text-green-700 dark:text-green-300 group-hover:text-green-900 dark:group-hover:text-green-100 transition-colors truncate">
      ...
      <div className="md:hidden fixed top-24 right-2 z-40 space-y-2">
        <div className={`transition-all duration-300 ease-in-out ${
          ...
          {/* Componente Último Evento mobile - apenas na página inicial */}
          {isHomePage && (
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border border-green-200/50 dark:border-green-700/50 rounded-lg px-2 py-1.5 shadow-lg max-w-[160px] mb-2">
              <div className="flex items-center space-x-1.5">
                <div className="flex-shrink-0">
                  <div className="w-5 h-5 bg-green-100 dark:bg-green-800 rounded-full flex items-center justify-center">
                    <Calendar size={10} className="text-green-600 dark:text-green-400" />
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-xs font-semibold text-green-800 dark:text-green-200">
